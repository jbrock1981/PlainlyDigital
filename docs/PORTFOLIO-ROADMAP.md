...
### Quick-revenue validation track — Fraus + Pillarly
Shipped landing-page scaffolds 2026-04-17. Owner must execute the validation gate before any MVP work.

- [ ] Create Neon project (separate) for Fraus and Pillarly
- [ ] Create Meta Pixels (separate) for each
- [ ] Purchase `<your fraus domain>` and `pillarly.com` / `.app`
...
- [ ] Pillarly: $50 / 48hr Meta ad set A (60+, "medication management") + ad set B (40–55, "caring for aging parent")
- [ ] **Green-light gate:** Fraus CPL < $3; Pillarly CPL < $4
- [ ] **Reposition gate:** Fraus CPL > $8; Pillarly CPL > $10
...
- Expo React Native mobile (voice-first for Pillarly; paste/OCR-first for Fraus)
- RevenueCat for payments
- Apple + Google sign-in added **after** first successful App Store + Play Store listings

Pillarly adds two hard legal gates before public launch:
...
- [ ] Fraus + Pillarly domain + trademark lock-in (contingent on validation)
- [ ] Evaluate graduating repos into a PlainlyDigital GitHub Organization
- [ ] Consider bundling Fraus + Pillarly into a single "Fraus Plus" family-safety subscription if both validate (share the family-plan back end)
